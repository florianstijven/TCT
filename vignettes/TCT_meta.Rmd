...
formula = formula(ADAScog_integer ~ arm_time + us(time_int | SubjId))
data_test$SubjId = as.factor(data_test$SubjId)
data_test$time_int = as.factor(data_test$time_int)
formula = stats::update.formula(old = formula, . ~ .)
mmrm_fit = mmrm::mmrm(formula = formula,
                      data = data_test,
                      reml = TRUE)
```
...
are described in the function documentation. 
  ...
  * For inference on the estimated acceleration factors, we use the `"contrast"` based
    inference.
...
* The `inference` argument specifies the method of inference. We choose once `"contrast"` and
once `"least-squares"`.
  * For `"contrast"` based inference, if we leave the `weights` unspecified, the weights
   will be estimated data-adaptively as to minimize the standard error of the common 
...
```{r}
TCT_common_contrast = TCT_meta_common(
